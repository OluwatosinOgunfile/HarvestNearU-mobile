...
export default function FarmDetails() {
  ...
  return <Screen scrollRef={pageRef} refreshing={false} onRefresh={load}>
    ...
    <View style={styles.hero}>
      <Pressable accessibilityLabel="Back to shop" hitSlop={12} onPress={leave} style={styles.heroBack}><ChevronLeft size={17} strokeWidth={2.8} color="#d7e4da" /><Text style={styles.heroBackText}>BACK TO SHOP</Text></Pressable>
    ...
    <View style={styles.content} onLayout={event=>setContentY(event.nativeEvent.layout.y)}>
      ...
      {data.recommendations?.length ? <>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recommendations}>
          {data.recommendations.map(item => <Pressable key={item.id} onPress={() => router.push({ pathname:'/farms/[id]', params:{ id:item.farm_id, listingId:item.id } })} style={[styles.recommendation, { backgroundColor:theme.surface, borderColor:theme.border }]}>
