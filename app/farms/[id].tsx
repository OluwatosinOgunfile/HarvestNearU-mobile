...
import { BadgeCheck, Bell, BellRing, ChevronLeft, ChevronRight, ExternalLink, Leaf, MapPin, Navigation, PackageOpen, Plus, Star, Store, Truck } from 'lucide-react-native';
...
type FarmData = {
  ...
  outOfStock?: { id:string; name:string; unit:string; unit_price_kobo:number; category:string; image?:string }[];
...
export default function FarmDetails() {
  ...
  const { theme, products, add, user } = useApp();
  const pageRef = useRef<ScrollView>(null);
  const didFocusListing = useRef(false);
  const [contentY,setContentY] = useState(0);
  const [listingsY,setListingsY] = useState(0);
  ...
  const loadWatched = useCallback(async () => {
    ...
    try { setWatched((await api<{ watching:string[] }>('/api/restock-alerts')).watching || []); }
    catch { setWatched([]); }
  }, [user]);
  async function toggleAlert(listingId: string) {
    if (!user) { router.push('/account'); return; }
    const watching = !watched.includes(listingId);
    setAlertBusy(listingId); setAlertError('');
    setWatched(current => watching ? [...current, listingId] : current.filter(value => value !== listingId));
    try { await api('/api/restock-alerts', { method:'PUT', body:JSON.stringify({ listingId, watching }) }); }
    catch (reason) {
      setWatched(current => watching ? current.filter(value => value !== listingId) : [...current, listingId]);
      setAlertError((reason as Error).message);
    } finally { setAlertBusy(''); }
  }
  useFocusEffect(useCallback(() => { void load(); void loadWatched(); }, [load, loadWatched]));
  useEffect(()=>{didFocusListing.current=false},[id,listingId]);
  useEffect(()=>{if(!data||!listingId||!contentY||!listingsY||didFocusListing.current)return;didFocusListing.current=true;setTimeout(()=>pageRef.current?.scrollTo({y:Math.max(0,contentY+listingsY-76),animated:true}),120)},[contentY,data,listingId,listingsY]);
  ...
  return <Screen scrollRef={pageRef} refreshing={false} onRefresh={load}>
    <Header />
    <View style={styles.hero}>
      <Tap accessibilityLabel="Back to shop" hitSlop={12} onPress={leave} style={styles.heroBack}><ChevronLeft size={17} strokeWidth={2.8} color="#d7e4da" /><Text style={styles.heroBackText}>BACK TO SHOP</Text></Tap>
    ...
    <View style={styles.content} onLayout={event=>setContentY(event.nativeEvent.layout.y)}>
      ...
      {hasMap?<View style={[styles.mapCard,{backgroundColor:theme.surface,borderColor:theme.border}]}><View style={styles.mapHeading}><View style={{flex:1}}><Text style={[styles.eyebrow,{color:theme.primary}]}>FARM LOCATION</Text><Text style={[styles.mapTitle,{color:theme.text}]}>Find {data.farm.name}</Text><Text style={[styles.mapAddress,{color:theme.muted}]}>{data.farm.address_text}, {data.farm.city}, {data.farm.state}</Text></View><View style={[styles.factIcon,{backgroundColor:theme.surfaceAlt}]}><Navigation size={20} color={theme.primary}/></View></View><View style={styles.mapPreview}><WebView source={{uri:mapEmbed}} originWhitelist={['https://www.openstreetmap.org']} javaScriptEnabled domStorageEnabled scrollEnabled={false} setSupportMultipleWindows={false} style={styles.mapWebView}/></View><Tap onPress={()=>void openDirections()} style={[styles.directionButton,{backgroundColor:theme.primary}]}><Text style={{color:theme.primaryText,fontWeight:'900'}}>Open free directions</Text><ExternalLink size={18} color={theme.primaryText}/></Tap><Text style={[styles.attribution,{color:theme.muted}]}>Map data (c) OpenStreetMap contributors</Text></View>:null}

      <View onLayout={event=>setListingsY(event.nativeEvent.layout.y)}><SectionHeading eyebrow="AVAILABLE NOW" title={`Produce from ${data.farm.name}`} count={`${data.listings.length} active`} theme={theme} /></View>
      {data.listings.length ? <View style={styles.grid}>{data.listings.map(item => {const marketProduct=products.find(product=>product.id===item.id);const selected=item.id===listingId;return <View key={item.id} style={[styles.listing, { backgroundColor:theme.surface, borderColor:selected?theme.primary:theme.border,borderWidth:selected?2:1 }]}>
        <Image source={{ uri:absoluteUrl(item.image) }} style={styles.image} contentFit="cover"  transition={220}/>
        <View style={styles.listingBody}><Text style={[styles.category, { color:theme.primary }]}>{item.category.toUpperCase()}</Text><Text style={[styles.listingName, { color:theme.text }]} numberOfLines={2}>{titleCase(item.name)}</Text><Text style={[styles.stock, { color:theme.muted }]}>{item.stock} {item.unit}{item.stock === 1 ? '' : 's'} available</Text><View style={[styles.listingPrice, { borderTopColor:theme.border }]}><View style={styles.priceCopy}><Money value={Number(item.unit_price_kobo) / 100} style={{ color:theme.text, fontSize:17 }} /><Text style={{ color:theme.muted, fontSize:11 }}>/ {item.unit}</Text></View><Tap accessibilityLabel={`Add ${item.name} to basket`} disabled={!marketProduct||item.stock<1} onPress={()=>marketProduct&&add(marketProduct)} style={[styles.listingAdd,{backgroundColor:theme.primary,opacity:marketProduct&&item.stock>0?1:.45}]}><Plus size={17} color={theme.primaryText}/><Text style={{color:theme.primaryText,fontWeight:'900'}}>Add</Text></Tap></View></View>
      ...
      {data.outOfStock?.length ? <>
        <SectionHeading eyebrow="SOLD OUT FOR NOW" title="Tell me when it returns" count={String(data.outOfStock.length)} theme={theme} />
        <Text style={[styles.restockIntro, { color:theme.muted }]}>We will send a notification and an email the moment this farm restocks.</Text>
        <View style={styles.grid}>{data.outOfStock.map(item => {const watching=watched.includes(item.id);return <View key={item.id} style={[styles.listing, styles.listingSoldOut, { backgroundColor:theme.surface, borderColor:theme.border }]}>
          <Image source={{ uri:absoluteUrl(item.image) }} style={[styles.image, styles.imageSoldOut]} contentFit="cover"  transition={220}/>
          <View style={styles.listingBody}><Text style={[styles.category, { color:theme.muted }]}>{item.category.toUpperCase()}</Text><Text style={[styles.listingName, { color:theme.text }]} numberOfLines={2}>{titleCase(item.name)}</Text><Text style={[styles.stock, { color:theme.muted }]}>Out of stock</Text><View style={[styles.listingPrice, { borderTopColor:theme.border }]}><View style={styles.priceCopy}><Money value={Number(item.unit_price_kobo) / 100} style={{ color:theme.muted, fontSize:17 }} /><Text style={{ color:theme.muted, fontSize:11 }}>/ {item.unit}</Text></View><Tap accessibilityRole="button" accessibilityState={{ selected:watching }} accessibilityLabel={watching ? `Stop watching ${item.name}` : `Tell me when ${item.name} is back`} disabled={alertBusy===item.id} onPress={()=>void toggleAlert(item.id)} style={[styles.listingAdd,{backgroundColor:watching?theme.surfaceAlt:theme.primary,borderColor:theme.primary,borderWidth:watching?1:0,opacity:alertBusy===item.id?.6:1}]}>{watching?<BellRing size={16} color={theme.primary}/>:<Bell size={16} color={theme.primaryText}/>}<Text style={{color:watching?theme.primary:theme.primaryText,fontWeight:'900',fontSize:12}}>{watching?'Watching':'Notify me'}</Text></Tap></View></View>
        </View>})}</View>
      ...
      {data.recommendations?.length ? <>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.recommendations}>
          {data.recommendations.map(item => <Tap key={item.id} onPress={() => router.push({ pathname:'/farms/[id]', params:{ id:item.farm_id, listingId:item.id } })} style={[styles.recommendation, { backgroundColor:theme.surface, borderColor:theme.border }]}>
            <Image source={{ uri:absoluteUrl(item.image) }} style={styles.recommendationImage} contentFit="cover"  transition={220}/>
