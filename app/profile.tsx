...
export default function UserProfile(){
  ...
  return <Screen refreshing={loading} onRefresh={load}>
    ...
    {loading?<ActivityIndicator color={theme.primary} style={{marginTop:80}}/>:<View style={styles.content}>
      ...
      {data?.user.role==='consumer'?<Pressable onPress={()=>router.push('/farm-new' as never)} style={[styles.upgrade,{backgroundColor:theme.surfaceAlt,borderColor:theme.primary}]}><View style={[styles.upgradeIcon,{backgroundColor:theme.primary}]}><Store size={22} color={theme.primaryText}/></View><View style={{flex:1}}><Text style={[styles.locationTitle,{color:theme.text}]}>Become a farmer</Text><Text style={{color:theme.muted,lineHeight:19}}>Add your first farm while keeping this account and purchase history.</Text></View><Plus size={20} color={theme.primary}/></Pressable>:null}
