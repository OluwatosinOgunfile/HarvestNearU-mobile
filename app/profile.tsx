...
type ProfileData = {
  ...
  addresses: { line1: string; city: string; state: string }[];
  stats: {
    total_orders: number;
...
export default function UserProfile() {
  ...
  async function choosePicture() {
    ...
    if (asset.fileSize && asset.fileSize > 3 * 1024 * 1024) {
      setMessage("Choose a profile picture smaller than 3 MB.");
      return;
    }
    setUploading(true);
    try {
      if (!asset.base64)
        throw new Error(
          "The selected picture could not be read. Please choose it again.",
        );
      const response = await api<{ avatarUrl: string }>("/api/profile/avatar", {
        method: "POST",
        body: JSON.stringify({
      ...
      setData((current) =>
        current
          ? {
              ...current,
              user: { ...current.user, avatar_url: response.avatarUrl },
            }
          : current,
      );
        ...
        <View style={styles.content}>
          ...
          >
            ...
            <View style={styles.identityCopy}>
              ...
              <Text style={[styles.identityRole, { color: theme.muted }]}>
                {data?.user.role.replace(/^./, (value) => value.toUpperCase())}{" "}
                account
              </Text>
            </View>
            <Pressable
          ...
          <View style={styles.stats}>
            ...
            <Stat
              theme={theme}
              label="Completed"
              value={data?.stats.completed_orders || 0}
            />
            <Stat
              theme={theme}
              label="Farms"
              value={data?.stats.farms_supported || 0}
            />
          </View>
          <View
            style={[
          ...
          >
            ...
            <View>
              ...
              <Text style={[styles.creditValue, { color: theme.text }]}>
                ₦
                {((data?.storeCredit.balance_kobo || 0) / 100).toLocaleString(
                  "en-NG",
                )}
              </Text>
            </View>
          ...
          >
            <Title
              theme={theme}
              icon={<UserRound size={20} color={theme.primary} />}
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ color: theme.muted }}>
                {data?.addresses[0]
                  ? `${data.addresses[0].line1}, ${data.addresses[0].city}, ${data.addresses[0].state}`
          ...
          {data?.user.role === "consumer" ? (
            ...
            >
              <View
                style={[styles.upgradeIcon, { backgroundColor: theme.primary }]}
              >
                <Store size={22} color={theme.primaryText} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={[styles.locationTitle, { color: theme.text }]}>
                  Become a farmer
                ...
                <Text style={{ color: theme.muted, lineHeight: 19 }}>
                  Add your first farm while keeping this account and purchase
                  history.
                </Text>
              </View>
          ...
          {data?.user.role === "farmer" ? (
            ...
            >
              ...
              <Text
                style={{ color: theme.primaryText, fontWeight: "900", flex: 1 }}
              >
                Add another farm
              </Text>
            </Pressable>
            ...
            >
              ...
              {data.farms.map((farm) => (
                <Pressable
                  ...
                  style={[styles.farm, { borderTopColor: theme.border }]}
                >
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.farmName, { color: theme.text }]}>
                      {farm.name}
                    </Text>
                    <Text style={{ color: theme.muted }}>
                  ...
                  <View style={{ alignItems: "flex-end" }}>
                    <Text style={{ color: theme.primary, fontWeight: "800" }}>
                      {farm.verification_status.replace(/^./, (c) =>
                        c.toUpperCase(),
                      )}
                    </Text>
                    <Text style={{ color: theme.muted }}>
                      {Number(farm.average_rating || 0).toFixed(1)} ·{" "}
...
const styles = StyleSheet.create({
  ...
  title: { fontFamily: "Georgia_Regular", fontSize: 29 },
  ...
  identityCopy: { flex: 1 },
  identityName: { fontFamily: "Georgia_Bold", fontSize: 20 },
  identityRole: { fontSize: 12, fontWeight: "700", marginTop: 4 },
  photoButton: {
    width: 44,
    height: 44,
    borderRadius: 13,
  ...
  message: { padding: 12, borderWidth: 1, borderRadius: 10, marginTop: 12 },
  stats: { flexDirection: "row", gap: 8, marginTop: 13 },
  stat: { flex: 1, padding: 12, borderWidth: 1, borderRadius: 13 },
  statValue: { fontSize: 21, fontWeight: "800" },
  small: { fontSize: 10, fontWeight: "800", marginTop: 3 },
  credit: {
    marginTop: 12,
    padding: 15,
    borderWidth: 1,
  ...
  creditValue: { fontSize: 21, fontWeight: "800" },
  ...
  locationTitle: { fontSize: 15, fontWeight: "800", marginBottom: 4 },
  farm: {
    paddingVertical: 13,
    borderTopWidth: 1,
    flexDirection: "row",
  ...
  farmName: { fontSize: 14, fontWeight: "800" },
});
