...
export default function Payouts() {
  ...
  async function openAccount(){if(!data)return;setBusy(true);setError('');try{const result=await api<{account:Account;banks:Bank[]}>(`/api/farmer/payout-account?farmId=${data.farm.id}`);setAccount(result.account);setBanks(result.banks||[]);setBank(null);setAccountNumber('');setResolved(null);setConfirmed(false);setAccountOpen(true)}catch(reason){setError((reason as Error).message)}finally{setBusy(false)}}
  ...
  async function saveAccount(){if(!data||!bank||!resolved||!confirmed)return;setBusy(true);setError('');try{const result=await api<{account:Account}>('/api/farmer/payout-account',{method:'PUT',body:JSON.stringify({farmId:data.farm.id,bankCode:bank.code,bankName:bank.name,accountNumber,confirmedAccountName:resolved.accountName})});setAccount(result.account);setAccountOpen(false);setResolved(null);setConfirmed(false)}catch(reason){setError((reason as Error).message);setResolved(null);setConfirmed(false)}finally{setBusy(false)}}
