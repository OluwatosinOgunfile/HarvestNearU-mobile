...
import { Check, ChevronDown, ChevronLeft, CircleDollarSign, Clock3, CreditCard, Printer, Send, X } from 'lucide-react-native';
...
export default function Payouts() {
  ...
  const [accountOpen,setAccountOpen]=useState(false);const [account,setAccount]=useState<Account>(null);const [banks,setBanks]=useState<Bank[]>([]);const [bank,setBank]=useState<Bank|null>(null);const [accountNumber,setAccountNumber]=useState('');const [resolved,setResolved]=useState<{accountName:string;accountLast4:string}|null>(null);const [resolving,setResolving]=useState(false);const [confirmed,setConfirmed]=useState(false);const [password,setPassword]=useState('');const [passwordRequired,setPasswordRequired]=useState(false);
  ...
  async function openAccount(){if(!data)return;setBusy(true);setError('');try{const result=await api<{account:Account;banks:Bank[];passwordRequired?:boolean}>(`/api/farmer/payout-account?farmId=${data.farm.id}`);setAccount(result.account);setBanks(result.banks||[]);setBank(null);setAccountNumber('');setResolved(null);setConfirmed(false);setPassword('');setPasswordRequired(Boolean(result.passwordRequired));setAccountOpen(true)}catch(reason){setError((reason as Error).message)}finally{setBusy(false)}}
  // Ask the bank who owns the account so the farmer can check the name before anything is saved.
  async function checkAccount(){
    if(!data||!bank||accountNumber.length!==10)return;
    setResolving(true);setResolved(null);setConfirmed(false);setError('');
    try{
      const result=await api<{accountName:string;accountLast4:string}>('/api/farmer/payout-account/resolve',{method:'POST',body:JSON.stringify({farmId:data.farm.id,bankCode:bank.code,accountNumber})});
      setResolved(result);
    }catch(reason){setError((reason as Error).message)}
    finally{setResolving(false)}
  }
  async function saveAccount(){if(!data||!bank||!resolved||!confirmed||(passwordRequired&&!password))return;setBusy(true);setError('');try{const result=await api<{account:Account}>('/api/farmer/payout-account',{method:'PUT',body:JSON.stringify({farmId:data.farm.id,bankCode:bank.code,bankName:bank.name,accountNumber,confirmedAccountName:resolved.accountName,password:passwordRequired?password:undefined})});setAccount(result.account);setAccountOpen(false);setResolved(null);setConfirmed(false);setPassword('')}catch(reason){setError((reason as Error).message);setPassword('');setResolved(null);setConfirmed(false)}finally{setBusy(false)}}
  ...
  return (
    <Screen refreshing={busy} onRefresh={() => load()}>
      <View style={styles.header}>
        <Tap accessibilityLabel="Go back" onPress={() => router.back()} style={[styles.back, { borderColor: theme.border, backgroundColor: theme.surface }]}>
        ...
        {data ? (
          <>
            <View style={[styles.balance, { backgroundColor: theme.surface, borderColor: theme.border }]}>
              ...
              <Tap disabled={busy || available <= 0} onPress={() => void submit()} style={({ pressed }) => [styles.request, { backgroundColor: theme.primary }, (busy || available <= 0) && styles.disabled, pressed && available > 0 && styles.pressed]}>
            ...
            {requests.length ? requests.map((request) => (
              <View key={request.id} style={[styles.history, { backgroundColor: theme.surface, borderColor: theme.border }]}>
                ...
                <View style={styles.historyActions}><Text style={[styles.status, { color: theme.primary, backgroundColor: theme.surfaceAlt }]}>{label(request.status)}</Text><Tap accessibilityLabel="Print payout statement" onPress={()=>void printPayout(request.id)} style={[styles.print,{borderColor:theme.border}]}><Printer size={16} color={theme.primary}/></Tap></View>
              </View>
            )) : (
            ...
            <Tap onPress={()=>void openAccount()} style={[styles.accountButton,{borderColor:theme.primary}]}><CreditCard size={18} color={theme.primary}/><Text style={{color:theme.primary,fontWeight:'900'}}>Configure payout account</Text></Tap>
          </>
        ) : busy ? <ActivityIndicator color={theme.primary} style={styles.loader} /> : null}
      </View>
      <Modal transparent visible={accountOpen} animationType="slide" onRequestClose={()=>setAccountOpen(false)}><View style={styles.modalBackdrop}><View style={[styles.modal,{backgroundColor:theme.surface,borderColor:theme.border}]}><View style={styles.modalHead}><View><Text style={[styles.eyebrow,{color:theme.primary}]}>PAYOUT ACCOUNT</Text><Text style={[styles.modalTitle,{color:theme.text}]}>{data?.farm.name}</Text></View><Tap onPress={()=>setAccountOpen(false)} style={[styles.modalClose,{borderColor:theme.border}]}><X size={20} color={theme.text}/></Tap></View>{account?.account_last4?<View style={[styles.currentAccount,{backgroundColor:theme.surfaceAlt}]}><CreditCard size={20} color={theme.primary}/><View><Text style={{color:theme.text,fontWeight:'800'}}>{account.account_name}</Text><Text style={{color:theme.muted}}>{account.bank_name} · ending {account.account_last4}</Text></View></View>:null}<Text style={[styles.fieldLabel,{color:theme.muted}]}>Bank</Text><ScrollView style={[styles.bankList,{borderColor:theme.border}]}>{banks.map(item=><Tap key={item.code} onPress={()=>{setBank(item);setResolved(null);setConfirmed(false)}} style={[styles.bankRow,{borderColor:theme.border,backgroundColor:bank?.code===item.code?theme.surfaceAlt:theme.surface}]}><Text style={{color:theme.text}}>{item.name}</Text>{bank?.code===item.code?<ChevronDown size={16} color={theme.primary}/>:null}</Tap>)}</ScrollView><Text style={[styles.fieldLabel,{color:theme.muted}]}>Account number</Text><TextInput value={accountNumber} onChangeText={value=>setAccountNumber(value.replace(/\D/g,'').slice(0,10))} keyboardType="number-pad" placeholder="10-digit NUBAN account" placeholderTextColor={theme.muted} style={[styles.input,{color:theme.text,borderColor:theme.border,backgroundColor:theme.surfaceAlt}]}/>{error?<Text style={[styles.error,{color:dark?'#ffb4a8':'#a84335'}]}>{error}</Text>:null}{resolved?<View style={[styles.resolved,{backgroundColor:theme.surfaceAlt,borderColor:theme.primary}]}><Text style={[styles.resolvedLabel,{color:theme.primary}]}>THIS ACCOUNT BELONGS TO</Text><Text style={[styles.resolvedName,{color:theme.text}]}>{resolved.accountName}</Text><Text style={{color:theme.muted,fontSize:12,lineHeight:18,marginTop:4}}>Ending {resolved.accountLast4}. Payouts for this farm will be sent here.</Text><Tap accessibilityRole="checkbox" accessibilityState={{checked:confirmed}} onPress={()=>setConfirmed(value=>!value)} style={[styles.confirmRow,{borderTopColor:theme.border}]}><View style={[styles.checkbox,{borderColor:confirmed?theme.primary:theme.border,backgroundColor:confirmed?theme.primary:'transparent'}]}>{confirmed?<Check size={14} color={theme.primaryText}/>:null}</View><Text style={{color:theme.text,fontWeight:'700',flex:1}}>This is the correct account name</Text></Tap></View>:null}{resolved&&passwordRequired?<><Text style={[styles.fieldLabel,{color:theme.muted}]}>Your password</Text><TextInput value={password} onChangeText={setPassword} secureTextEntry autoComplete="current-password" textContentType="password" placeholder="Confirm it is you" placeholderTextColor={theme.muted} style={[styles.input,{color:theme.text,borderColor:theme.border,backgroundColor:theme.surfaceAlt}]}/><Text style={{color:theme.muted,fontSize:12,lineHeight:17,marginTop:6}}>Changing where your earnings are sent needs your password, even though you are already signed in.</Text></>:null}{resolved?<Tap disabled={busy||!confirmed||(passwordRequired&&!password)} onPress={()=>void saveAccount()} style={[styles.request,{backgroundColor:theme.primary},(busy||!confirmed||(passwordRequired&&!password))&&styles.disabled]}>{busy?<ActivityIndicator color={theme.primaryText}/>:<Text style={[styles.requestText,{color:theme.primaryText}]}>Save payout account</Text>}</Tap>:<Tap disabled={resolving||!bank||accountNumber.length!==10} onPress={()=>void checkAccount()} style={[styles.request,{backgroundColor:theme.primary},(resolving||!bank||accountNumber.length!==10)&&styles.disabled]}>{resolving?<ActivityIndicator color={theme.primaryText}/>:<Text style={[styles.requestText,{color:theme.primaryText}]}>Check account name</Text>}</Tap>}</View></View></Modal>
    </Screen>
  );
}

const styles = StyleSheet.create({
  resolved:{marginTop:14,padding:13,borderWidth:1,borderRadius:12},resolvedLabel:{fontSize:10,fontWeight:'900',letterSpacing:.6},resolvedName:{fontSize:17,fontWeight:'800',marginTop:5},confirmRow:{marginTop:12,paddingTop:12,borderTopWidth:1,flexDirection:'row',alignItems:'center',gap:10,minHeight:44},checkbox:{width:22,height:22,borderWidth:2,borderRadius:6,alignItems:'center',justifyContent:'center'},
