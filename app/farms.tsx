...
export default function Farms() {
  ...
  const matching = useMemo(() => !term ? farms : farms.filter(farm =>
    `${farm.name} ${farm.location} ${farm.categories.join(' ')}`.toLowerCase().includes(term)), [farms, term]);
  // Only a screenful or two is mounted at a time, so a long directory does not hold every card and
  // its photograph in memory at once.
  const shown = useMemo(() => matching.slice(0, visibleCount), [matching, visibleCount]);
  const stocked = matching.filter(farm => farm.inStock > 0).length;
  ...
  return <Screen refreshing={loading} onRefresh={load}>
    ...
    <View style={styles.content}>
      ...
      <Text style={[styles.subtitle, { color:theme.muted }]}>{matching.length} farms{matching.length ? ` · ${stocked} with produce ready today` : ''}.</Text>
      <View style={[styles.search, { backgroundColor:theme.surface, borderColor:theme.border }]}>
        <Search size={20} color={theme.muted} />
        <TextInput value={query} onChangeText={value => { setQuery(value); setVisibleCount(20); }} placeholder="Search farms, towns, or produce" placeholderTextColor={theme.muted} style={[styles.input, { color:theme.text }]} />
      </View>
      {loading && !farms.length ? <ActivityIndicator color={theme.primary} style={{ marginTop:50 }} />
        : error ? <View style={[styles.empty, { backgroundColor:theme.surface, borderColor:theme.border }]}>
            ...
            <Tap onPress={() => void load()} style={[styles.retry, { backgroundColor:theme.primary }]}><Text style={{ color:theme.primaryText, fontWeight:'800' }}>Try again</Text></Tap>
          </View>
        : !matching.length ? <View style={[styles.empty, { backgroundColor:theme.surface, borderColor:theme.border }]}>
        ...
        : <View style={styles.list}>{shown.map(farm => <Tap key={farm.id} accessibilityRole="link" accessibilityLabel={`${farm.name}, ${farm.inStock} in stock`} onPress={() => router.push({ pathname:'/farms/[id]', params:{ id:farm.id } })} style={[styles.card, { backgroundColor:theme.surface, borderColor:theme.border }]}>
          ...
          </Tap>)}{shown.length < matching.length ? <Tap onPress={() => setVisibleCount(count => count + 20)} style={[styles.loadMore, { backgroundColor:theme.surface, borderColor:theme.border }]}><Text style={{ color:theme.primary, fontWeight:'900' }}>Show more farms</Text><Text style={{ color:theme.muted, fontSize:11 }}>{matching.length - shown.length} remaining</Text></Tap> : null}</View>}
    </View>
  </Screen>;
}
...
const styles = StyleSheet.create({
  ...
  list:{gap:11},loadMore:{minHeight:56,marginTop:3,borderWidth:1,borderRadius:13,alignItems:'center',justifyContent:'center',gap:2},card:{borderWidth:1,borderRadius:15,overflow:'hidden',flexDirection:'row',gap:12,padding:12},image:{width:96,height:96,borderRadius:11},body:{flex:1,minWidth:0,gap:5},nameRow:{flexDirection:'row',alignItems:'center',gap:5},name:{flexShrink:1,fontFamily:'serif',fontSize:18,fontWeight:'600'},metaRow:{flexDirection:'row',alignItems:'center',gap:4},meta:{flexShrink:1,fontSize:12},
