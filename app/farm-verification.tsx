...
export default function FarmVerification() {
  ...
  return <Screen>
    <View style={[styles.topbar, { backgroundColor:theme.surface, borderBottomColor:theme.border }]}>
      <Pressable accessibilityLabel="Go back" hitSlop={12} onPress={() => router.canGoBack() ? router.back() : router.replace('/workspace')} style={[styles.back, { backgroundColor:theme.background, borderColor:theme.border }]}><ChevronLeft size={22} color={theme.text} /></Pressable>
    ...
    <View style={styles.content}>
      {!data ? <ActivityIndicator color={theme.primary} style={{ marginTop:40 }} /> : <>
        ...
        {data.farm.exempt ? null : <>
          ...
          <Field label="Date of birth (YYYY-MM-DD)" theme={theme} value={form.dateOfBirth} editable={!locked} onChange={value => setForm({ ...form, dateOfBirth:value })} placeholder="1990-04-21" />
          ...
          {!locked ? <Pressable disabled={Boolean(busy)} onPress={() => void save(false)} style={[styles.secondary, { borderColor:theme.primary }]}>
          ...
          {[...data.policy.requiredDocuments, ...data.policy.optionalDocuments].map(type => {
            ...
            return <Pressable key={type} disabled={locked || Boolean(busy) || !submission?.id} onPress={() => void upload(type)} style={[styles.document, { backgroundColor:theme.surface, borderColor:done ? theme.primary : theme.border, opacity:locked || !submission?.id ? .6 : 1 }]}>
          ...
          {!locked ? <Pressable disabled={Boolean(busy) || missing.length > 0} onPress={() => void save(true)} style={[styles.primary, { backgroundColor:theme.primary, opacity:missing.length ? .5 : 1 }]}>
...
const styles = StyleSheet.create({
  ...
  row:{flexDirection:'row',gap:10},field:{marginTop:12},label:{fontSize:10,fontWeight:'900',letterSpacing:.8,marginTop:14,marginBottom:7},
