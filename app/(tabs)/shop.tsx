...
import { Check, RefreshCw, Search, SlidersHorizontal, Sprout, X } from 'lucide-react-native';
...
export default function Shop() {
  const { theme, products, categories: catalogue, loading, error, refresh } = useApp();
  ...
  const categories = useMemo(() => {
    const counts = new Map(catalogue.map(item => [item.name, item.listings]));
    const names = catalogue.length ? catalogue.map(item => item.name) : [...new Set(products.map(product => product.category))];
    return [{ name:'All', listings:products.length }, ...names.map(name => ({ name, listings:counts.get(name) ?? products.filter(product => product.category === name).length }))];
  }, [products, catalogue]);

  useEffect(() => {
    const timer = setTimeout(() => {
      if (!requestedCategory) { setCategory('All'); return; }
      const match = categories.find(item => item.name.toLowerCase() === requestedCategory.toLowerCase());
      if (match) { setCategory(match.name); setVisibleCount(12); }
    }, 0);
  ...
  return <Screen refreshing={loading} onRefresh={refresh}>
    ...
    <View style={styles.content}>
      ...
      <View style={[styles.search, { backgroundColor:theme.surface, borderColor:theme.border }]}>
        ...
        <Tap accessibilityLabel="Open produce filters" onPress={() => setFiltersOpen(value => !value)} style={[styles.filterButton, { backgroundColor:filtersOpen ? theme.primary : theme.surfaceAlt }]}>
      ...
      {searchInput.length >= 3 && (intentLoading || localSearchMatches.length === 0) ? <View style={[styles.intent, { backgroundColor:theme.surfaceAlt, borderColor:theme.border }]}>
        <View><Image source={require('@/assets/images/amara-avatar.png')} style={styles.intentAvatar} transition={220}/>{intentLoading ? <View style={[styles.intentSpinner,{backgroundColor:theme.surface}]}><ActivityIndicator size={11} color={theme.primary}/></View> : null}</View>
        <View style={styles.intentCopy}><Text style={[styles.intentName, { color:theme.primary }]}>AMARA</Text><Text style={[styles.intentTitle, { color:theme.text }]}>{intentLoading ? 'I am looking for broader matches...' : intentEnhanced ? intentExplanation : intentExplanation}</Text>{shown.length > 0 ? <Text style={[styles.intentTerms, { color:theme.muted }]}>I found available produce through related search terms.</Text> : null}</View>
      </View> : null}
      ...
      <View style={styles.categoryArea}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryChips}>
          {categories.map(item => { const active = item.name === category; return <Tap key={item.name} accessibilityRole="button" accessibilityState={{ selected:active }} onPress={() => { setCategory(item.name); setVisibleCount(12); }} style={[styles.categoryChip, !item.listings && !active && styles.categoryChipEmpty, { backgroundColor:active ? theme.primary : theme.surface, borderColor:active ? theme.primary : theme.border }]}>
            <Text style={[styles.categoryChipText, { color:active ? theme.primaryText : theme.text }]}>{item.name === 'All' ? 'All produce' : item.name}</Text>
            <Text style={[styles.categoryChipCount, { color:active ? theme.primaryText : theme.muted }]}>{item.listings || '0'}</Text>
          </Tap>; })}
        </ScrollView>
      </View>
      {loading ? <ActivityIndicator color={theme.primary} style={{ marginTop:50 }} />
        : error ? <Empty theme={theme} title="Could not load the market" copy={error} retry={refresh} />
          : shown.length === 0 ? <Empty theme={theme} title="No matching harvests" copy="Clear the search or adjust your filters." />
            : <><View style={styles.list}>{visible.map(product => <ProductCard key={product.id} product={product} fullWidth compact />)}</View>{visible.length < shown.length ? <Tap onPress={() => setVisibleCount(count => count + 12)} style={[styles.loadMore, { backgroundColor:theme.surface, borderColor:theme.border }]}><Text style={{ color:theme.primary, fontWeight:'900' }}>Load more harvests</Text><Text style={{ color:theme.muted, fontSize:11 }}>{shown.length - visible.length} remaining</Text></Tap> : null}</>}
    </View>
  </Screen>;
}
