    ...
    <View style={styles.content}>
      ...
      <View style={styles.categoryArea}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryChips}>
          {categories.map(item => <Pressable key={item} accessibilityRole="button" accessibilityState={{ selected:item === category }} onPress={() => { setCategory(item); setVisibleCount(12); }} style={[styles.categoryChip, { backgroundColor:item === category ? theme.primary : theme.surface, borderColor:item === category ? theme.primary : theme.border }]}>
            <Text style={[styles.categoryChipText, { color:item === category ? theme.primaryText : theme.text }]}>{item === 'All' ? 'All produce' : item}</Text>
          </Pressable>)}
        </ScrollView>
      </View>
