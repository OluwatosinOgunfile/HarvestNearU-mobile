  ...
  return <Screen refreshing={loading} onRefresh={refresh}>
    ...
    <View style={styles.content}>
      ...
      <View style={[styles.search, { backgroundColor:theme.surface, borderColor:theme.border }]}>
        ...
        <Pressable accessibilityLabel="Open produce filters" onPress={() => setFiltersOpen(value => !value)} style={[styles.filterButton, { backgroundColor:filtersOpen ? theme.primary : theme.surfaceAlt }]}>
      ...
      {searchInput.length >= 3 && (intentLoading || localSearchMatches.length === 0) ? <View style={[styles.intent, { backgroundColor:theme.surfaceAlt, borderColor:theme.border }]}>
        <View><Image source={require('@/assets/images/amara-avatar.png')} style={styles.intentAvatar}/>{intentLoading ? <View style={[styles.intentSpinner,{backgroundColor:theme.surface}]}><ActivityIndicator size={11} color={theme.primary}/></View> : null}</View>
      ...
      <View style={styles.categoryArea}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryChips}>
          {categories.map(item => { const active = item.name === category; return <Pressable key={item.name} accessibilityRole="button" accessibilityState={{ selected:active }} onPress={() => { setCategory(item.name); setVisibleCount(12); }} style={[styles.categoryChip, !item.listings && !active && styles.categoryChipEmpty, { backgroundColor:active ? theme.primary : theme.surface, borderColor:active ? theme.primary : theme.border }]}>
